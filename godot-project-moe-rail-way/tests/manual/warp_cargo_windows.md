...
16. Reproduce the held Warp-latch report: contact a live Warp during the drag, continue beyond it, wait through an anchor refresh, backtrack before it, and draw a new branch while still holding left. Confirm the prefix cannot detach from the live Warp, the new suffix bends from its exact center, and candidate-local locked pieces retired by the backtrack do not reappear merely because replacement input reuses their serials. Backtrack once more until the Warp itself is the endpoint and confirm no stale exit-support rejection freezes the prior preview. Expiry releases only that gesture-local editing floor without immediately rewriting the accepted route. Repeat with two contacted Warps to confirm the deepest live contact wins and expiry falls back to the earlier live contact.
