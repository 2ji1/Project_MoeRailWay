...
## Exact-center local-corner visual addendum

This addendum applies to the `feature/warp-exact-center-local-corners` candidate. The user owns the visual and control-feel verdict; automated geometry and presentation tests cannot complete this section.
...
At every supported window size, inspect accepted exact-center turns representing the visible `1x1`, `2x2`, and `3x3` ownership scales when the fixed-seed run or mouse route makes them available. For each observed scale, confirm all of the following:

1. Long portions of the owner remain visually straight.
2. Curvature is limited to the actual direction-change neighborhoods.
3. No owner-wide S-shaped excursion remains before or after the Warp center.
4. The rendered track crosses the literal center of the Warp marker.
5. The accepted ownership scale does not visibly shrink merely to satisfy the exact anchor.
