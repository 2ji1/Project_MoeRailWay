...
5. Exact-anchor presence changes only the coincident knot's contact constraint. It does not select a different owner, move route records, or alter topology.
6. Preserve exactly `nominal_length_cells * 16 + 1` samples, uniform-index nominal sampling, nonzero endpoint travel, entry and exit headings, footprint containment, candidate fallback, one owner per serial, and deterministic replay.
