...
Run the complete project automated suite after the focused tests. The Windows manual play must visibly demonstrate: a normal-speed draw whose already-solid head becomes a larger solid curve; no provisional ghost styling; a train that does not jump when the player continues drawing; rejected input retaining the last valid track; endpoint-only cancellation; and one-cell-at-a-time recovery.
