...
If a later recovered route record is itself the departure coordinate, that coordinate becomes the active predecessor boundary again and is not immediately reservable a second time. It becomes eligible again only after recovery advances beyond it. Active-cell uniqueness, orthogonal continuity, geometry overlap rejection, train locking, and inventory conservation remain unchanged. A retained locked piece blocks candidate geometry only with cells still owned by its active records; recovered cells in that piece's historical footprint no longer block reuse. The full locked centerline, immutable ledger identity, and train-sampling interval remain intact.
