...
기존 Risk 통합 검증이 앱 내부에 합성 Warp 레코드를 직접 넣는 경우에는 해당 레코드에 회사 ID와 운임을 명시해 마이그레이션합니다. 이 검증을 위해 Task 2 허용 경로에 Risk 통합 러너를 포함하며, 실제 앱이 옛 전역 운임을 읽도록 되돌리지는 않습니다.

Task 3에서 실제 앱의 배송 운임이 즉시 현금에 반영되므로 Risk 통합 검증의 종료 현금 기대값도 같은 러너에서 갱신합니다. 이 검증을 위해 Task 3 허용 경로에도 Risk 통합 러너를 포함하며, 기존 유료 행동의 무환불·정확히 한 번 차감 계약은 그대로 유지합니다.
