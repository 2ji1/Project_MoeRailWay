...
> 이 문서는 사용자 검토를 위한 한국어 브리핑입니다. 에이전트가 사용하는 영어 정본은 `docs/superpowers/specs/2026-08-15-warp-rail-prototype-design.md`이며, 선로·열차 세부 정본은 `docs/superpowers/specs/2026-08-16-prototype-track-train-design.md`입니다.
...
- 수동 적재·하역과 완성 선로의 무료 수동 회수. 단, `proto/04`의 유료 조기 철거는 허용한다.
- 워프 운행 중 대출
...
- 플레이어는 하나로 이어진 예약 경로의 현재 끝에서만 선로를 연장한다.
- 선로를 그리면 실제 건설보다 먼저 길이를 예약하고 보유 선로 총량에서 즉시 차감한다.
- 예약·미건설 선로는 완성 선로 끝에서부터 정해진 속도로 순차 건설되며, 열차는 완성 선로만 달린다.
- 예약·미건설 선로를 우클릭하면 클릭 지점부터 예약 끝까지 무료로 취소하고 그 길이를 즉시 반환한다.
- 기차 뒤의 고정된 회수 기준점을 지난 선로는 자동으로 사라지고, 그 길이만큼 보유량으로 즉시 돌아온다. 현금 환급은 없다.
- 이미 회수되어 사라진 경로의 좌표는 다시 자유롭게 통과할 수 있다.
- `proto/04`부터 완성·미주행 선로의 클릭 지점 이후를 유료 철거할 수 있다. 주행 완료 후방 선로도 활성 후방 시작점부터 클릭 지점까지 같은 비용으로 조기 철거할 수 있다.
