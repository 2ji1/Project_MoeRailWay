# 워프 레일 무한 생존 프로토타입 설계 — 사용자 검토용

> 이 문서는 사용자 검토를 위한 한국어 브리핑입니다. 에이전트가 사용하는 영어 정본은 `docs/superpowers/specs/2026-08-15-warp-rail-prototype-design.md`이며, 현재 선로 세부 정본은 `docs/superpowers/specs/2026-08-24-prototype-grid-track-amendment-design.md`입니다. `2026-08-16` 선로 문서는 완료된 `prototype-m3` 이력입니다.
...
무작위로 생성되고 유한한 수명을 가진 워프 화물을 상대로, 멈추지 않는 기차 한 대의 앞에 제한된 선로 칸을 한 줄로 계속 배치하면서 운송 실적과 회사의 현금흐름을 함께 생존시키는 타임어택 한붓그리기 게임이다.
...
- 수동 적재·하역과 완성 선로의 무료 수동 회수. 유료 조기 철거는 `proto/05-risk-investment`에서 정의한다.
- 워프 운행 중 대출
...
- 플레이어는 하나로 이어진 격자 노선의 현재 끝에서만 선로를 연장한다.
- 드래그가 실제로 통과한 상하좌우 인접 칸을 순서대로 예약하며, 시스템은 대각선 지름길이나 목적지 자동 경로를 만들지 않는다.
- 새로 예약한 고유 칸 한 개가 선로 재고 한 칸을 정확히 한 번 소비한다. 이미 소유한 칸이 큰 커브로 다시 분류되어도 추가 차감하지 않는다.
- 시스템은 예약 칸을 직선 또는 `1×1`·`2×2`·`3×3` 커브로 표시하며 각각 명목 노선거리 `1`·`3`·`5`칸을 갖는다. 워프 칸 접촉과 가까운 커브의 겹침은 더 작은 커브를 강제할 수 있다.
- 미건설 노선은 반투명 고스트이며 건설 전까지 다시 배치될 수 있다. 어떤 선형의 첫 칸이 건설을 시작하면 그 선형 전체가 잠긴다.
- 한 칸은 `예약 고스트 → 건설 중 → 완성` 순서로 변하며 열차는 연속된 완성 구간에만 진입한다.
- 잠기지 않은 고스트 칸을 우클릭하면 그 칸부터 뒤쪽 고스트 끝까지 무료로 취소하고 제거한 칸마다 재고 한 칸을 반환한다.
- 기차 뒤 회수 간격을 지난 선로는 노선 순서대로 한 칸씩 사라지고 재고로 돌아온다. 현금 환급은 없으며 회수된 칸은 다시 사용할 수 있다.
- 격자 기준 유료 철거와 입체교차는 `proto/05-risk-investment`가 별도 정의한다. 입체교차는 분기점이나 합류점이 아니다.
- 보유량이 부족해 더 배치할 수 없어도 기차는 멈추지 않는다. 다음 칸이 완성되기 전에 선로 끝에 도달하면 운행이 종료된다.
- 기본 선로 총량은 세션 중 현금으로 늘릴 수 있지만, 추가분은 워프 종료와 함께 사라진다.

이 규칙의 핵심은 매번 최단거리만 찾는 것이 아니라, 곧 회수될 선로 칸, 정수 재고, 커브 점유영역, 워프 접점과 자기교차 가능성을 읽으며 한 줄을 계속 유지하는 것이다.
...
- 워프 안에서 구매할 수 있는 항목은 선로 총량 증가, 적재칸 증가, 입체교차, 완성·미주행 선로 수정과 완성 선로 철거다.
...
각 고정 물리 틱의 판정 순서는 다음과 같이 고정한다.

1. 우클릭 예약 취소 또는 승인된 유료 철거 처리
2. 좌클릭 드래그 입력을 선로 예약으로 반영
3. 실제 선로 건설과 출차 준비 완료 판정
4. 기차 이동과 완성 선로 끝 도달 여부를 계산하고 종료 요청 기록
5. 출발지·목적지 통과 판정과 적재·배송 처리
6. 위험 지형 피해와 내구도 0 판정
7. 회수 가능한 후방 선로 제거와 재고 반환
8. 워프 쌍 수명 만료 처리
9. 세션 시간 만료 처리
10. 발생한 종료 요청의 우선순위 확정
11. 단 한 번의 정산
12. 읽기 전용 화면 상태를 발행한 뒤 완료 결과를 한 번만 발행
